...
const CalendarOutlet = () => {
    ...
    useEffect(() => {
        const calendarDays = document.querySelectorAll('.react-calendar__month-view__weekdays__weekday abbr');
    ...
    const tabItems = [
        {
            key: '1',
            label: `Events`,
            children: events.map(event => <AllEvents date={event.date} month={event.month} name={event.name} />),
        },
        {
            key: '2',
            label: `My Sessions`,
